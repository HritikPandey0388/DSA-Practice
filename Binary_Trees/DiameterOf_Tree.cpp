...
int Diameter(Node* root){
    ...
    int currDiameter = (height(root->left) + height(root->right) + 1);     // curr Node diameter
    int ldiameter = Diameter(root->left);                                  // left diameter
    int rdiameter = Diameter(root->right);                                 // right diameter

    return max(currDiameter,max(ldiameter,rdiameter));  // ans 
}
